...
Библиотека предоставляет best-shipping DataStore, который возвращает лучший по приоритету способ доставки. Может возвращать несколько способов если продукт будет сразу в нескольких поставщиков.

Для работы необходимо указать `CATALOG_API_URL="http://catalog"` в env. 
